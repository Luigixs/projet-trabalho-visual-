document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('cancel').addEventListener('click', () => {
        calendarContainer.style.display = 'none'; // Apenas fecha o calendário
    });
    ...
    function addRow() {
        const codigoInput = document.querySelector('.input-group.fornecedor input').value; // Captura o código
        const periodoInput = `${startDate ? startDate.toLocaleDateString() : ''} - ${endDate ? endDate.toLocaleDateString() : ''}`; // Captura o período usando as datas selecionadas
        const valorInput = document.querySelector('.input-group.produto input').value; // Captura o valor
        ...
        rows.forEach((row) => {
            ...
            if (cells[0].innerText === "") { // Verifica se a célula do código está vazia
                cells[0].innerText = codigoInput; // Preenche com o código
                cells[1].innerText = periodoInput; // Preenche com o período
                cells[2].innerText = valorInput; // Preenche com o valor
                foundEmptyRow = true;
                return; // Sai do loop após preencher uma linha
            }
        });

        if (!foundEmptyRow) {
            document.getElementById('error-message').style.display = 'block'; // Exibe erro se não houver linhas vazias
        } else {
            document.getElementById('error-message').style.display = 'none'; // Oculta mensagem de erro
        }
    }

    // Evento para o botão de adicionar linha
    document.getElementById('add-row-button').addEventListener('click', addRow); // Certifique-se de que o ID do botão de adicionar linha está correto
});
